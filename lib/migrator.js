...
const { basename, dirname, extname, join, parse } = require('path').posix;
...
module.exports = async function(args) {
  ...
  const { 'import-image': import_image, 'default-category': defaultCatArg } = args;
  const tomd = new TurndownService({ headingStyle: 'atx', codeBlockStyle: 'fenced' });
  const { config, log } = this;
  const { default_category: defaultCatCfg, source_dir, post_asset_folder } = config;
    ...
    for (const item of feed.items) {
      ...
      if (type === 'attachment') {
        ...
        if (!post_asset_folder) {
          imagePath = imagePath.startsWith('/') ? imagePath : join('/', imagePath);
        } else {
      ...
      let categories;
      if (newPostCats.length) {
        categories = nestCats(newPostCats);
      } else if (defaultCatArg) {
        categories = [[defaultCatArg]];
      } else if (defaultCatCfg) {
        categories = [[defaultCatCfg]];
      }
      ...
      if (Object.keys(images).length && rImg.test(content)) {
        ...
        const myAsyncFn = async (matched, wpImg) => {
          ...
          if (path) {
            ...
            const newPath = path.includes('/') ? join(dirname(path), basename(wpImg)) : basename(wpImg);
            const resizePath = import_image !== 'original' ? newPath : path;
